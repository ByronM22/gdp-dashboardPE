...
if tipo_calculo == 'Cálculo con unidades':
    ...
    total_gastos = st.number_input('Ingrese el total de gastos ($)', min_value=0, value=3000, step=100, key="total_gastos")
    porcentaje_costo_variable = st.number_input('Ingrese el porcentaje del costo variable (%)', min_value=0.0, max_value=100.0, value=40.0, step=0.1, key="porcentaje_costo_variable") / 100
    ...
    if st.button("Mostrar Gráfica"):
        ...
        cantidades = np.arange(0, 2000, 1)
        ingresos_totales_graf = total_ventas * cantidades / 2000
        costos_totales_graf = total_gastos + (porcentaje_costo_variable * total_ventas * cantidades / 2000)
    ...
    # Inputs
    total_gastos = st.number_input('Ingrese el total de gastos ($)', min_value=0, value=3000, step=100, key="total_gastos")
    total_ventas = st.number_input('Ingrese el total de ventas ($)', min_value=0, value=10000, step=100, key="total_ventas")
    ...
    if st.button("Mostrar Gráfica"):
        plt.figure(figsize=(10,6))
        plt.pie([total_gastos, total_ventas-total_gastos], labels=['Gastos', 'Utilidad'], autopct='%1.1f%%', colors=['orange', 'green'])
        plt.title('Distribución de Gastos y Utilidad')
        st.pyplot(plt)
